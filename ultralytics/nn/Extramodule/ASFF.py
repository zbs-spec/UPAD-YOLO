...
class ASFFV5(nn.Module):
    ...
    def forward(self, x):  # l,m,s
        ...
        # 确保不同尺度的特征图尺寸一致
        target_size = level_0_resized.shape[2:]  # 获取目标尺寸（假设使用level_0_resized作为基准）

        # 进行插值操作，确保其他特征图与level_0_resized的尺寸一致
        if level_1_resized.shape[2:] != target_size:
            level_1_resized = F.interpolate(level_1_resized, size=target_size, mode='bilinear', align_corners=False)
        if level_2_resized.shape[2:] != target_size:
            level_2_resized = F.interpolate(level_2_resized, size=target_size, mode='bilinear', align_corners=False)
